...
public class CheckManager {
    ...
    public boolean startCheck(CommandSender staff, Player target) {
        ...
        if (plugin.getExtendedConfig().useTeleport() && plugin.getExtendedConfig().getCheckLocation() != null) {
            // Используем безопасную телепортацию
            safelyTeleportPlayer(target, plugin.getExtendedConfig().getCheckLocation(), "телепортация на проверку");
        }
        ...
        if (plugin.getPluginConfig().notifyStaff()) {
            String notifyMessage = "&e" + target.getName() + " &7был вызван на проверку администратором &e" + 
        ...
        // Отправляем сообщение игроку о проверке
        String message = plugin.getPluginConfig().getCheckMessage("target", 
                "&cВы были вызваны на проверку. Пожалуйста, выполняйте указания администратора!");
        plugin.getMessageManager().sendMessage(target, message);
        
        // Отправляем сообщение с инструкцией AnyDesk вместо Discord
        if (!plugin.getExtendedConfig().getDiscordLink().isEmpty()) {
            plugin.getMessageManager().sendMessage(target, 
                    "&eОтправьте свой код AnyDesk через команду /anydesk");
        }
        
        // Логируем событие в Discord, если включено
        if (plugin.getExtendedConfig().isDiscordLoggingEnabled()) {
            String staffName = staff instanceof Player ? ((Player) staff).getName() : "Console";
            DiscordWebhook.sendCheckStartLog(plugin, target.getName(), staffName);
        }
    ...
    public boolean endCheck(CommandSender staff, Player target, boolean isCheating, String cheat) {
        ...
        if (previousLocation == null) {
            plugin.getLogger().warning("endCheck: Отсутствует предыдущая локация для игрока " + target.getName());
        }
        
        // Размораживаем игрока
        plugin.getFreezeManager().unfreezePlayer(target);
        
        // Логируем события в Discord
        if (plugin.getExtendedConfig().isDiscordLoggingEnabled()) {
            DiscordWebhook.sendCheckFinishLog(plugin, target.getName(), staffName, isCheating, cheat);
        }
        
        // Обрабатываем результат проверки
        if (isCheating) {
            // Игрок использовал читы - выполняем соответствующие действия
            handleCheatingPlayer(staff, target, cheat);
        } else {
            // Игрок не использовал читы - возвращаем его на прежнюю локацию
            handleCleanPlayer(staff, target, previousLocation);
        }
    ...
    public boolean requestScreenshare(CommandSender staff, Player target) {
        ...
        // Логируем запрос скриншота в Discord
        if (plugin.getExtendedConfig().isDiscordLoggingEnabled()) {
            String staffName = staff instanceof Player ? ((Player) staff).getName() : "Console";
            org.shavin.cheaterCheck.utils.DiscordWebhook.sendScreenshareLog(plugin, target.getName(), staffName);
        }
    ...
    public boolean handlePlayerQuit(UUID playerUuid) {
        ...
        if (player != null) {
            ...
            // Получаем информацию о проверяющем администраторе
            ...
            if (plugin.getPluginConfig().autoBanOnQuit()) {
                ...
                // Логируем событие в Discord, если логирование включено
                if (plugin.getExtendedConfig().isDiscordLoggingEnabled()) {
                    DiscordWebhook.sendPlayerQuitLog(plugin, playerName, staffName, true);
                }
            } else {
                ...
                // Логируем событие в Discord, если логирование включено
                if (plugin.getExtendedConfig().isDiscordLoggingEnabled()) {
                    DiscordWebhook.sendPlayerQuitLog(plugin, playerName, staffName, false);
                }
            }
        }
    ...
    private void safelyTeleportPlayer(Player player, Location location, String reason) {
        ...
        // Проверяем, что мир локации существует и загружен
        if (location.getWorld() == null) {
            plugin.getLogger().severe("safelyTeleportPlayer: Ошибка телепортации - мир локации не существует");
            return;
        }
        ...
        // Клонируем локацию для предотвращения проблем с несинхронным доступом
        final Location safeLoc = location.clone();
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                try {
                    ...
                    if (originalMode != GameMode.SURVIVAL && plugin.getExtendedConfig().useTeleport()) {
                    ...
                    plugin.getLogger().info("safelyTeleportPlayer: Результат телепортации для " + 
                            ...
                            " в мир: " + safeLoc.getWorld().getName() + 
                            ", x: " + safeLoc.getX() + 
                            ", y: " + safeLoc.getY() + 
                            ", z: " + safeLoc.getZ());
                    ...
                    if (!success) {
                        ...
                        new BukkitRunnable() {
                            ...
                            public void run() {
                                if (player.isOnline()) {
                                    player.setMetadata("cheatercheck_teleport", new FixedMetadataValue(plugin, true));
                                    boolean retrySuccess = player.teleport(safeLoc);
    ...
    public boolean setTimePause(CommandSender staff, Player target, boolean pause) {
        ...
        // Логируем операцию паузы в Discord
        if (plugin.getExtendedConfig().isDiscordLoggingEnabled()) {
            String staffName = staff instanceof Player ? ((Player) staff).getName() : "Console";
            org.shavin.cheaterCheck.utils.DiscordWebhook.sendTimerPauseLog(plugin, target.getName(), staffName, pause);
        }
    ...
    public boolean addTime(CommandSender staff, Player target, int seconds) {
        ...
        // Логируем добавление времени в Discord
        if (plugin.getExtendedConfig().isDiscordLoggingEnabled()) {
            String staffName = staff instanceof Player ? ((Player) staff).getName() : "Console";
            org.shavin.cheaterCheck.utils.DiscordWebhook.sendAddTimeLog(plugin, target.getName(), staffName, seconds);
        }
    ...
    /**
     * Обрабатывает игрока, уличенного в использовании читов
     *
     * @param staff Администратор, проводящий проверку
     * @param target Игрок, уличенный в читах
     * @param cheat Название чита
     */
    private void handleCheatingPlayer(CommandSender staff, Player target, String cheat) {
    ...
    /**
     * Обрабатывает чистого игрока после проверки
     *
     * @param staff Администратор, проводящий проверку
     * @param target Чистый игрок
     * @param previousLocation Предыдущая локация игрока
     */
    private void handleCleanPlayer(CommandSender staff, Player target, Location previousLocation) {
        // Устанавливаем метаданные об истечении срока проверки
        markPlayerAsRecentlyChecked(target);
