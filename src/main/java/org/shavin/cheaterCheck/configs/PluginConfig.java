...
public class PluginConfig {
    ...
    // Настройки логирования в Discord
    ...
    public void loadConfig() {
        ...
        discordLoggingEnabled = config.getBoolean("discord.logging.enabled", false);
        discordWebhookUrl = config.getString("discord.webhook.url", "");
        discordAvatarUrl = config.getString("discord.webhook.avatar-url", "https://i.imgur.com/Bzyxsoz.png");
        discordUsername = config.getString("discord.webhook.username", "CheaterCheck");
    }
    ...
    private Location loadCheckLocation() {
        ...
        if (world == null) {
            ...
            // Попробуем получить стандартный мир сервера
            world = Bukkit.getWorlds().isEmpty() ? null : Bukkit.getWorlds().get(0);
            if (world == null) {
                plugin.getLogger().severe("loadCheckLocation: Не удалось найти ни один загруженный мир! Телепортация не будет работать.");
                return null;
            }
            plugin.getLogger().warning("loadCheckLocation: Используем мир по умолчанию '" + world.getName() + "' вместо '" + worldName + "'");
        }
        
        // Проверяем наличие всех необходимых координат
        if (!locSection.contains("x") || !locSection.contains("y") || !locSection.contains("z")) {
            plugin.getLogger().warning("loadCheckLocation: Не указаны все необходимые координаты (x, y, z)");
            return null;
        }
    ...
    /**
     * Устанавливает настройку телепортации при проверке
     *
     * @param enabled Включить или выключить телепортацию
     */
    public void setUseTeleport(boolean enabled) {
        this.useTeleport = enabled;
        config.set("check.teleport.enabled", enabled);
        plugin.saveConfig();
    }
    ...
    /**
     * Проверяет, включено ли логирование в Discord
     *
     * @return true, если логирование в Discord включено
     */
    public boolean isDiscordLoggingEnabled() {
        return discordLoggingEnabled && !discordWebhookUrl.isEmpty();
    }
    
    /**
     * Получает URL вебхука Discord
     *
     * @return URL вебхука Discord
     */
    ...
    /**
     * Устанавливает URL вебхука Discord
     *
     * @param webhookUrl URL вебхука Discord
     */
    public void setDiscordWebhookUrl(String webhookUrl) {
        this.discordWebhookUrl = webhookUrl;
        config.set("discord.webhook.url", webhookUrl);
        config.set("discord.logging.enabled", !webhookUrl.isEmpty());
        plugin.saveConfig();
    }
    
    /**
     * Получает URL аватара для вебхука Discord
     *
     * @return URL аватара для вебхука Discord
     */
    ...
    /**
     * Получает имя пользователя для вебхука Discord
     *
     * @return Имя пользователя для вебхука Discord
     */
