...
public class Config {
    ...
    public List<String> getAllowedCommands() {
        ...
        // Загружаем список разрешенных команд
        if (config.isList("freeze.allowed-commands")) {
            allowedCommands.addAll(config.getStringList("freeze.allowed-commands"));
        }
        
        // Убедимся, что команда anydesk всегда разрешена
        if (!allowedCommands.contains("anydesk")) {
            allowedCommands.add("anydesk");
        }
