...
public class CheckCommand implements CommandExecutor, TabCompleter {
    ...
    public CheckCommand(CheaterCheck plugin) {
        this.plugin = plugin;
        this.subCommands = Arrays.asList("help", "start", "stop", "finish", "reload", "bypass", "debug", "forcecheck", "list", "timestop", "timeadd", "webhook", "tpcheck");
        this.cheatSubCommands = Arrays.asList("add", "remove", "list");
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        // Проверяем наличие прав на использование команды
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "start":
                return handleStartCheck(sender, args);
            case "stop":
                return handleStopCheck(sender, args);
            case "finish":
                return handleFinishCheck(sender, args);
            case "reload":
            ...
            case "timestop":
                return handleTimeStopCommand(sender, args);
            case "timeadd":
                return handleTimeAddCommand(sender, args);
            case "debug":
                return handleDebugCommand(sender);
            case "forcecheck":
                return handleForceCheckCommand(sender, args);
            case "webhook":
                return handleWebhookCommand(sender, args);
            case "tpcheck":
                return handleTpCheckCommand(sender);
            default:
                plugin.getMessageManager().sendMessage(sender, "&cНеизвестная подкоманда. Используйте &e/check help &cдля просмотра доступных команд.");
                return true;
        }
    }
    ...
    private boolean showHelp(CommandSender sender) {
        plugin.getMessageManager().sendMessage(sender, "&e===== Команды CheaterCheck =====");
        plugin.getMessageManager().sendMessage(sender, "&e/check help &7- Показать эту справку");
        ...
        plugin.getMessageManager().sendMessage(sender, "&e/check reload &7- Перезагрузить конфигурацию");
        plugin.getMessageManager().sendMessage(sender, "&e/check bypass add/remove/list <игрок> &7- Управление списком байпаса");
        plugin.getMessageManager().sendMessage(sender, "&e/check list &7- Показать список активных проверок");
        plugin.getMessageManager().sendMessage(sender, "&e/check timestop [игрок] &7- Остановить таймер проверки");
        plugin.getMessageManager().sendMessage(sender, "&e/check timeadd <секунды> [игрок] &7- Добавить время к проверке");
        plugin.getMessageManager().sendMessage(sender, "&e/check tpcheck &7- Установить точку телепортации для проверки");
        ...
        plugin.getMessageManager().sendMessage(sender, "&e/check webhook <url> &7- Установить Discord вебхук для логирования");
        return true;
    }
     ...
     */
    private boolean handleStartCheck(CommandSender sender, String[] args) {
        // Проверяем права
     ...
     * @param args Аргументы команды
     */
    private boolean handleStopCheck(CommandSender sender, String[] args) {
        // Проверяем права
     ...
     * @param args Аргументы команды
     */
    private boolean handleFinishCheck(CommandSender sender, String[] args) {
        // Проверяем права
     ...
     * @param args Аргументы команды
     */
    private boolean handleForceCheckCommand(CommandSender sender, String[] args) {
     ...
     */
    private boolean handleTimeStopCommand(CommandSender sender, String[] args) {
        // Проверка прав
     ...
     * @return true, если команда выполнена успешно
     */
    private boolean handleTimeAddCommand(CommandSender sender, String[] args) {
        // Проверка прав
        if (!sender.hasPermission("cheatercheck.admin") && !sender.hasPermission("cheatercheck.check")) {
            plugin.getMessageManager().sendNoPermissionMessage(sender);
            return true;
    ...
    /**
     * Обрабатывает команду установки вебхука Discord
     *
     * @param sender Отправитель команды
     * @param args Аргументы команды
     * @return true, если команда выполнена успешно
    ...
    private boolean handleWebhookCommand(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            plugin.getMessageManager().sendMessage(sender, "&cИспользование: /check webhook <url>");
            plugin.getMessageManager().sendMessage(sender, "&7Текущий вебхук: &e" + 
                    (plugin.getExtendedConfig().getDiscordWebhookUrl().isEmpty() ? "не установлен" : "установлен"));
            return true;
        }
        ...
        if (!webhookUrl.startsWith("https://discord.com/api/webhooks/")) {
            plugin.getMessageManager().sendMessage(sender, "&cНеверный URL вебхука Discord. URL должен начинаться с https://discord.com/api/webhooks/");
            return true;
        }
        
        // Устанавливаем URL вебхука
        plugin.getExtendedConfig().setDiscordWebhookUrl(webhookUrl);
        plugin.getMessageManager().sendMessage(sender, "&aВебхук Discord успешно установлен!");
        
        return true;
    }

    /**
     * Обрабатывает команду установки точки телепортации для проверки
     *
     * @param sender Отправитель команды
     * @return true, если команда выполнена успешно
     */
    private boolean handleTpCheckCommand(CommandSender sender) {
        ...
        if (!sender.hasPermission("cheatercheck.tpcheck")) {
            plugin.getMessageManager().sendNoPermissionMessage(sender);
            return true;
        }
        ...
        if (location == null || location.getWorld() == null) {
            plugin.getMessageManager().sendMessage(sender, "&cНе удалось получить корректное местоположение!");
            plugin.getLogger().severe("handleTpCheckCommand: Не удалось получить местоположение игрока " + player.getName());
            return true;
        }
        
        // Сохраняем текущие координаты для отображения перед изменением
        ...
        // Сохраняем локацию для проверки
        plugin.getExtendedConfig().saveCheckLocation(location);
        
        // Включаем телепортацию при проверке
        plugin.getExtendedConfig().setUseTeleport(true);
        
        // Проверяем, что локация была успешно сохранена
        Location savedLocation = plugin.getExtendedConfig().getCheckLocation();
        if (savedLocation == null || savedLocation.getWorld() == null) {
            plugin.getMessageManager().sendMessage(sender, 
                    "&cОшибка при сохранении локации! Проверьте журнал сервера.");
            plugin.getLogger().severe("handleTpCheckCommand: Не удалось сохранить локацию. " +
                    "Результат после сохранения: " + (savedLocation == null ? "null" : 
                    "мир: " + (savedLocation.getWorld() == null ? "null" : savedLocation.getWorld().getName())));
            return true;
        }
        
        // Отправляем сообщение об успешной установке точки
        plugin.getMessageManager().sendMessage(sender, 
                "&aТочка телепортации для проверки установлена на ваше текущее местоположение!");
        plugin.getMessageManager().sendMessage(sender, 
                "&7Мир: &e" + worldName + 
                "&7, X: &e" + String.format("%.2f", x) + 
                "&7, Y: &e" + String.format("%.2f", y) + 
                "&7, Z: &e" + String.format("%.2f", z));
        
        plugin.getLogger().info("Игрок " + player.getName() + " установил точку телепортации для проверки: " +
                "мир=" + worldName + ", x=" + x + ", y=" + y + ", z=" + z + ", yaw=" + yaw + ", pitch=" + pitch);
        ...
        if (args.length == 1) {
            ...
            switch (args[0].toLowerCase()) {
                ...
                case "bypass":
                    ...
                    break;
                case "webhook":
                    // Предлагаем текущий URL вебхука, если есть
                    if (sender.hasPermission("cheatercheck.webhook")) {
                        String currentUrl = plugin.getExtendedConfig().getDiscordWebhookUrl();
                        if (!currentUrl.isEmpty()) {
                            List<String> urls = new ArrayList<>();
                            urls.add(currentUrl);
                            StringUtil.copyPartialMatches(args[1], urls, completions);
                        }
                    }
                    break;
            }
